...
import {newShade, quickSort} from '../utils/utils';
...
const Barchart = ({
  ...
  const calculateWidth = () => {
    ...
    const gap_between_ticks = chartWidth / (data.length + 1) + 5;
    return {
      chartWidth,
      gap_between_ticks,
    };
  ...
  const render_x_axis_ticks = () => {
    ...
    return data.map((item, index) => {
      const x = x_margin + x_margin / 2 + gap_between_ticks * index;
  ...
  const render_y_axis_labels = () => {
    ...
    return yAxisLabels.map((item, index) => {
      ...
      const textValue = (min + (yMax / dataPoints) * index).toFixed(1);
      return (
  ...
  const render_barchart = () => {
    ...
    return data.map((item, index) => {
      const x = x_margin + x_margin / 2 + x_gap * index;
